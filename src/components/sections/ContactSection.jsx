...
export default function ContactSection({ variant = 'cabana' }) {
    ...
    const label = variant === 'cabana' ? t('contact.unitTypeCabana') : t('contact.unitTypeDepto')
        ...
        <section id="contacto" className="py-16 bg-slate-50">
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
                    <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-3">{t('contact.title')}</h2>
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    ...
                    <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-4">
                        ...
                        <div className="bg-gradient-to-br from-primary-500 to-primary-700 rounded-xl p-6 text-white">
                            <p className="font-bold text-lg mb-1">{t('contact.hoursTitle')}</p>
                            <p className="text-white/80 text-sm">{t('contact.hours')}</p>
                            <p className="text-white/70 text-xs mt-1">{t('contact.hoursNote')}</p>
                        </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl shadow-sm p-8">
                        {sent ? (
                            <div className="text-center py-12">
                                ...
                                <h3 className="font-bold text-slate-800 text-lg mb-2">{t('contact.sentTitle')}</h3>
                                <p className="text-slate-500 text-sm mb-5">{t('contact.sentText')}</p>
                                <button onClick={() => setSent(false)} className="text-sm text-primary-600 underline">{t('contact.sendAnother')}</button>
                            </div>
                        ) : (
                            <>
                                <h3 className="font-bold text-slate-800 text-lg mb-5">{t('contact.formTitle')}</h3>
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <input required type="text" placeholder={t('contact.namePlaceholder')}
                                    ...
                                    <select value={form.guests} onChange={e => setForm(f => ({ ...f, guests: e.target.value }))}
                                        ...
                                        {['1', '2', '3', '4', '5', '6+'].map(n => (
                                            <option key={n} value={n}>{n} {n === '1' ? t('contact.guestSingular') : t('contact.guestPlural')}</option>
                                        ))}
                                    </select>
                                    <textarea rows={4} placeholder={t('contact.messagePlaceholder', { type: label })}
